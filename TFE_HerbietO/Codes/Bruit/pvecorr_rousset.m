...
%     for i = 1 : 1 : size(simg,1)
%         for j = 1 : 1 : size(simg,2)
%             for k = 1 : 1 : size(simg,3)
%                 simg(i,j,k) = simg(i,j,k) + 30*(-1+2*rand);
%             end
%         end
%     end
    Noise = 10*randn(size(simg));
    simg = simg + Noise;
    %simg_noise = imnoise(simg, 'gaussian', 0 , 0.001); % bruit
    ssimg{ima} = simg;          %simg_noise; %A  simg
...
%ecrire dans un fichier txt l'activite corrige dans chaque region
